...
| JavaScript/TS | [lessons-javascript.md](lessons-javascript.md) | 26 | Promise不await / forEach+async / var闭包 / any传染 / fetch缺超时 / innerHTML XSS / prototype pollution / SSR hydration / ANSI字符串长度 / DOM运行时JS静默失效(textContent/中文id/sticky兜底) / \uXXXX字面量Edit匹配 / 展示值取展示层(收藏中文) / File System Access API无path/getParent / Write写入\x00→NUL变binary / number input step从min起步invalid / OpenAI兼容baseUrl /v1后缀重复拼接404 / localStorage控件类型迁移 |
