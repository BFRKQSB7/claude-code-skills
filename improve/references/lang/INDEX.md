...
| 语言 | 文件 | 教训数 | 重点领域 |
|------|------|--------|----------|
| Python | [lessons-python.md](lessons-python.md) | 18 | async create_task / 裸except / 可变默认参数 / 迭代改列表 / with资源 / pickle RCE / eval注入 / 明文协议 / PyInstaller sys.frozen / tooltip Toplevel不消失 |
| JavaScript/TS | [lessons-javascript.md](lessons-javascript.md) | 22 | Promise不await / forEach+async / var闭包 / any传染 / fetch缺超时 / innerHTML XSS / prototype pollution / SSR hydration / ANSI字符串长度 / DOM运行时JS静默失效(textContent/中文id/sticky兜底) / \uXXXX字面量Edit匹配 / 展示值取展示层(收藏中文) / File System Access API无path/getParent |
