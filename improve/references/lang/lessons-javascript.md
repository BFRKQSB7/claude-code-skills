...
### ★ [2026-08-26] 主功能入口藏进高级折叠 → 用户判定功能不存在 (置信度: high, 命中: 1)

**Rule**: 用户明确要求且会频繁使用的主功能入口必须默认可见；高级折叠只收纳低频调优项。
**Wrong**: 全局 OCR 范围已实现，但入口放在默认收起的“高级设置”里，用户反馈“没有找到入口”。
**Right**: 移动同一控件到主流程附近并做视觉分组；避免复制入口造成状态分叉。
**Why**: 功能可用性包含可发现性；默认不可见的入口对首次用户等同于不存在。
...
### ★★ [2026-08-26] 局部保存触发整区重绘 → 未提交表单草稿回到默认值 (置信度: high, 命中: 2)

**Rule**: 重建含未提交控件的 DOM 前先按稳定键抓取草稿，重建后恢复；后端局部更新只清理真正依赖已变字段的状态。
**Wrong**: 保存文字样式后重绘术语清单 → “自定义翻译”回到“保留原文”，区域译文也被无条件清空。
**Right**: 优先局部更新状态且不重建 DOM；必须重绘时先保存 mode/translation，重绘后恢复。
...
**Rule**: 异步操作结束时只能清理该操作自己占有的状态；不要在 `finally` 中无条件复位一个可能已被用户重新开启的共享 UI 模式。
**Wrong**: 第一次框选 OCR 的请求尚未完成，用户切换模型并再次开启框选；旧请求的 `finally { setDrawMode(false) }` 随后执行，把新框选静默关闭。
**Right**: 提交请求前立即结束本次框选，并在请求期间禁用冲突入口；同时快照请求所用的模型和值。若必须允许并发，用递增 generation/token，只允许当前请求清理当前状态。
**Why**: Promise 回调按完成时间执行，不知道共享状态是否已进入下一代；“确保清理”不等于“可以清理后来者的状态”。
