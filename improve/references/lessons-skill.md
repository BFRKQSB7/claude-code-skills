...
**再次**: 2026-07-26 — 日期 tag vs semver 不一致，删旧重建。2026-08-08 — html-guide 13 个 Release 正文+标题格式不统一，已批量统一；教训：统一格式**标题+正文一起查**，首发就定全模板，之后每版严格套用。
...
**Rule**: 发布到 GitHub 的任何仓库，README 不允许只有外语（英文/日文等）——必须含中文版。默认结构：**文件切换版**——根 `README.md` 放中文（默认，GitHub 自动渲染），英文在 `docs/en/README.md`，每个 README 标题正下方放语言切换条（见 `templates/readme-template.md`「多语言版本」）。**改 README 时中英两版必须同步更新**。
...
**再次**: 2026-08-09 先做单文件双语 → 08-10 用户更正为文件切换版，4 个已做的 README 全改版。教训：语言结构偏好先问清形式（单文件 vs 文件切换），别默认。
...
**再次**: 2026-07-10×4 — ①下载指南写前先开 Release 页核对实际文件名（50 系=13.x，不能凭记忆）②看清每条目完整描述：companion/cudart-*.dll 必下载非可选 ③简单路径排最前、CLI/源码后放+标「进阶可选」④预判小白会犯的错；直接给可下载脚本优于 400 行源码。
...
### ★★ [2026-08-13] 子 agent 32K 输出硬上限 → 逐条大列表任务爆量失败 (置信度: high, 命中: 1)

**Rule**: 子 agent 跑 `deepseek-v4-flash` 有 **32K 输出硬上限**，千级分片任务约 10-30% 会超量截断（`stop_reason: max_tokens`），失败 agent 长时间卡槽拖慢并发。大列表逐条任务**禁止稀疏契约**（「只输出变更条目」不可靠，会过度填充/过度翻译）。
**Wrong**: 提示词超市 321K 标签逐条过滤 5 版方案（Write 工具 / 稀疏 JSONL / 最终回复 JSONL / 每行 TSV）全栽在 32K 上限。
**Right**: 「**小分片 ≤300 条 + 每行一条紧凑 TSV + 禁联网（拿不准标 unknown）+ 从 workflow journal 回收 + 失败片重跑**」稳定，99% 覆盖率。契约式逐条输出：每行必出、字段只有改了才填。
**Why**: 2026-08-13 321K 标签过滤项目实测。关联「级联复审会橡皮图章」。

### ★★ [2026-08-13] subagent 级联复审 → 橡皮图章前序判定 (置信度: high, 命中: 1)

**Rule**: 大规模过滤/审核任务里，**让 subagent「二次复审」另一个 subagent 的判定容易橡皮图章**——它倾向继承上游结论而不批判。凡「前一轮 agent 已判断、后一轮 agent 纠错」的流程，不要默认后一轮有效。
**Wrong**: 321K 过滤 P3 联网复审应救回 P1 误杀标签，实际把 1girl / jewelry / closed_eyes 等核心常用标签全部盖章成噪声，靠人工逐条复查 379 条才救回 22 条。级联复审 ≈ 无审。
**Right**: 关键/高频条目（按 count 排序的头部）单独人工或独立 agent 抽查；复审 prompt 明确「**别信前序判定，自己判断**」+ 对抗性要求（列出反例）。
**Why**: 2026-08-13 321K 过滤项目实测。关联「子 agent 32K 上限」。
...
## ★ [2026-08-10] 裸 /improve 被反问子命令 → 打扰 (命中: 1)

**Rule**: 用户单独敲 `/improve`（无有效提示词）= **提醒加载 improve skill**：直接进入就绪态（Phase 1 教训已加载），**不反问**「学习？创建？更新？发布？反省？」。只有带明确子命令/主题才进对应流程。
**Wrong**: 裸 `/improve` → 反问子命令 → 用户实际只想让 skill 就位（2026-08-10 用户明示「无有效提示词 = 提醒加载」）
...
## ★ [2026-08-23] Skill 双向分叉却直接覆盖 → 新教训被旧副本删除 (置信度: high, 命中: 1)

**Rule**: 同步运行目录与备份仓前先 `git pull`，再做忽略 CRLF 的双向 diff；差异含双方独有提交时先判定基线并合并，禁止默认“运行目录一定更新”后整目录覆盖。
**Wrong**: 看到 8 个差异文件就把运行目录复制到备份 → 将备份新增的 5 条教训删除，并重新引入已脱敏的本机路径/端口。
**Right**: 用备份 HEAD 修复落后的运行目录 → 写入本次新教训 → 只同步这次确认过的文件 → diff 零差异后 commit+push。
**Why**: 自修改 skill 可能被多个会话分别更新；文件时间和目录角色不能证明谁更新，三方基线与内容 diff 才能防知识倒退。
