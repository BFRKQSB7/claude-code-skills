...
## ★★ [2026-08-22] 先暴露可操作状态、后持久化 → 后续操作并发覆盖临时文件 (置信度: high, 命中: 2)
...
**再次**: 2026-08-26 — 取消清理与审核状态持久化并发，清理器删掉正在原子替换的 `job.tmp`，随后 `replace` 偶发报文件不存在。生成文件清理必须把元数据正式文件和临时文件都列入保护名单，或与持久化共用同一把锁。
